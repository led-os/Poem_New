...
public class PinyinText extends View {
    ...
    public PinyinText(Context context,  AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public PinyinText(Context context,  AttributeSet attrs, int defStyleAttr) {
    ...
    private void measurePinyinText(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < mPinyinCompats.size(); i++) {
            ...
            if (lineLength + maxWidth + (col == 0 ? 0 : mHorizontalSpacing) > sizeWidth || compat.text.equals("~")) { // new row

                if (!compat.text.equals("~")) {
                    lineLength = maxWidth;
                } else {
                    lineLength = 0;
                }
            ...
            compat.pinyinRect.left = lineLength - maxWidth + (maxWidth - pinyinWidth) / 2;
            ...
            compat.textRect.left = lineLength - maxWidth + (maxWidth - textWidth) / 2;
...
//            if (newLine) {//做居中
//                //PinyinCompat c = compat;
//                int j = i-1;
//                PinyinCompat c = mPinyinCompats.get(j);
//                int dx = (measuredWidth - lineLength) / 4;
//                while (j >= 0 && !c.text.equals("~")) {
//                    c = mPinyinCompats.get(j);
//                    c.textRect.offset(dx, 0);
//                    c.pinyinRect.offset(dx, 0);
//                    j--;
//                }
//            }
        }
...
//            }
    ...
    private void drawPinyinAndText(Canvas canvas) {
        ...
        for (int i = 0; i < mPinyinCompats.size(); i++) {
            ...
            if ("~".equals(compat.text)) continue;
            // draw text
